        ...
        >
          {caseStudies.map((study) => (
            <motion.div
              ...
              whileHover={{ y: -3 }}
              className="bg-white rounded-xl3 border border-neutral-200 shadow-card hover:shadow-card-md overflow-hidden flex flex-col transition-all duration-300"
            >
              {/* Industry tag */}
              <div className="px-8 py-3 bg-neutral-50 border-b border-neutral-100">
              ...
              <div className="p-8 flex flex-col flex-1">
                {/* Challenge */}
                <div className="mb-5">
                  <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Challenge</span>
                  <p className="text-neutral-700 text-sm mt-1.5 leading-relaxed">{study.challenge}</p>
                </div>

                {/* Solution */}
                <div className="mb-6">
                  <span className="text-xs font-bold text-neutral-400 uppercase tracking-wider">Solution</span>
                  <p className="text-neutral-700 text-sm mt-1.5 leading-relaxed">{study.solution}</p>
                </div>

                {/* Metrics */}
                <div className="mt-auto grid grid-cols-3 gap-3 pt-6 border-t border-neutral-100">
                  {study.metrics.map((m) => (
                    <div key={m.label}>
                      <AnimatedCounter
                        value={m.value}
                      ...
                      />
                      <p className="text-xs text-neutral-400 mt-0.5">{m.label}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-4">
                  {study.techStack.map((t) => (
                    <span
                      key={t}
                      className="text-xs bg-neutral-100 text-neutral-500 px-2 py-0.5 rounded-md font-mono"
                    >
                      {t}
                    </span>
                  ))}
